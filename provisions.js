...
function escProvision(value){return String(value??'').replace(/[&<>"']/g,ch=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[ch]))}
...
async function bootProvisionGenealogy(){
  ...
  try{
    ...
    let activeId=families[0]?.id||'';

    function familyCard(family){
      return `<button type="button" class="card provision-family-card${family.id===activeId?' active':''}" data-family="${escProvision(family.id)}" aria-pressed="${family.id===activeId}"><span class="provision-family-label">${escProvision(family.label)}</span><h3>${escProvision(family.title)}</h3><p>${escProvision(family.summary)}</p><div class="provision-family-meta"><span>${family.roots?.length||0} indexed stages</span><b>Open genealogy →</b></div></button>`;
    }
    ...
    function renderGrid(query=''){
      ...
      grid.querySelectorAll('[data-family]').forEach(btn=>btn.addEventListener('click',()=>{activeId=btn.dataset.family;renderGrid(search?.value||'');renderDetail()}));
    }
