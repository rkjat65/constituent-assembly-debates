...
const navRoutes={Home:`${rootPrefix}index.html`,Sessions:`${rootPrefix}chronology.html`,Speakers:`${rootPrefix}speakers.html`,Topics:`${rootPrefix}themes.html`,Chronology:`${rootPrefix}chronology.html`,Documents:`${rootPrefix}documents.html`,Provisions:`${rootPrefix}provisions.html`,Search:`${rootPrefix}search.html`};
...
document.querySelectorAll('img').forEach(img=>{img.addEventListener('error',()=>{img.style.display='none';const parent=img.parentElement;if(parent&&!parent.querySelector('.portrait-fallback')){const fallback=document.createElement('div');fallback.className='portrait-fallback';fallback.textContent=img.alt?.replace('Custom illustrated archival portrait of ','').replace('Custom illustrated portrait of ','')||'Portrait asset';parent.appendChild(fallback)}},{once:true})});
...
// Turn the existing debate-page profile button into a real personality route.
const profileRoutes={'Dr. Sachchidananda Sinha':'sachchidananda-sinha.html','J. B. Kripalani':'jb-kripalani.html','Jawaharlal Nehru':'jawaharlal-nehru.html'};
document.querySelectorAll('.speaker-card').forEach(card=>{const name=card.querySelector('.speaker-copy h3')?.textContent.trim();const button=card.querySelector('.outline-btn');if(name&&button&&profileRoutes[name])button.addEventListener('click',()=>{window.location.href=`${rootPrefix}speakers/${profileRoutes[name]}`})});
...
// My Library is deliberately deferred until public indexing is stable.
