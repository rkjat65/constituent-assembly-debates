...
const files=walk(root);const htmlFiles=files.filter(f=>f.endsWith('.html'));const jsonFiles=files.filter(f=>f.endsWith('.json'));const failures=[];const checked=[];
...
const required=['index.html','chronology.html','speakers.html','themes.html','documents.html','provisions.html','search.html','search.js','explorer-detail.css','styles.css','app.js','assets/sachchidananda-sinha-illustrated.webp','assets/jb-kripalani-illustrated.webp','assets/nehru-illustrated.webp','sessions/1946-12-09.html','sessions/1946-12-10.html','sessions/1946-12-13.html','speakers/sachchidananda-sinha.html','speakers/jb-kripalani.html','speakers/jawaharlal-nehru.html','topics/rules-procedure.html','topics/objectives-resolution.html','data/session-1946-12-11.json','data/session-1946-12-12.json'];
for(const item of required){if(!fs.existsSync(path.join(root,item)))failures.push(`Required site file missing: ${item}`)}

if(failures.length){console.error('\nStatic archive integrity check FAILED:\n');for(const failure of failures)console.error(`  - ${failure}`);console.error(`\nChecked ${htmlFiles.length} HTML files, ${jsonFiles.length} JSON files and ${checked.length} local references.`);process.exit(1)}
console.log(`Static archive integrity check passed: ${htmlFiles.length} HTML files, ${jsonFiles.length} JSON files, ${checked.length} local references, ${required.length} required routes/assets.`);